...
class VideoState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (Assets.exists(leSource.replace(".webm", ".ogg"), MUSIC) || Assets.exists(leSource.replace(".webm", ".ogg"), SOUND) || Assets.exists(leSource.replace(".webm", ".mp3"), MUSIC) || Assets.exists(leSource.replace(".webm", ".mp3"), SOUND))
		{
            useSound = true;
            #if web
			vidSound = FlxG.sound.play(leSource.replace(".webm", ".mp3"));
            #else
            vidSound = FlxG.sound.play(leSource.replace(".webm", ".ogg"));
            #end
		}
        #if web
        GlobalVideo.setVid(new VideoHandler());
        #else
        GlobalVideo.setWebm(new WebmHandler());
        #end
        GlobalVideo.get().init1();
        GlobalVideo.get().init2();
